...
instance Show a => Show (Stream a) where
    show x = "[" ++ intercalate ", " (map show $ sTake 10 x) ++ ", ..."
...
sInterleave (x :> x1) x2 = x :> sInterleave x2 x1
...
ruler = ruler1 0
    where
        ruler1 n = sInterleave (sRepeat n) (ruler1 (n + 1))
...
{- Total time: 0.109s Total Memory in use: 20 MB -}
{- Отключенная оптимизация Total time: 0.719s Total Memory in use:  92 MB -}
minMaxSlow [] = Nothing
...
{- Total time: 0.625s Total Memory in use: 83 MB -}
{- Отключенная оптимизация Total time: 0.672s Total Memory in use: 88 MB -}
minMax [] = Nothing
minMax (x:xs) = let 
                  minMax1 p [] = Just p
                  minMax1 (x1,x2) (y:ys) = minMax1 (min x1 y,max x2 y) ys
                in minMax1 (x,x) xs
